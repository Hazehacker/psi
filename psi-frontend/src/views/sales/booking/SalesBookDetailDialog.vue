...
/* 生成下拉：携带数据并跳转 */
async function generateTo(type: 'sale' | 'purchase') {
  if (!props.record?.id) {
    ElMessage.error('单据ID不存在')
    return
  }
  ...
  try {
    ...
    if (type === 'sale') {
      // 获取生成销售单数据
      const response = await salesBookApi.getGenerateSaleOrderData(props.record.id)
      if (response.code === 10000) {
      ...
      // 获取生成采购订单数据
      const response = await salesBookApi.getGeneratePurchaseOrderData(props.record.id)
      if (response.code === 10000) {
        generateData = response.data
      } else {
        ElMessage.error(response.message || '获取生成数据失败')
    ...
    console.error('生成数据获取失败:', error)
    ElMessage.error('生成数据获取失败')
  }
}
